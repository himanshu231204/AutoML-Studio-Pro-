...
def test_sample_datasets_dict_structure() -> None:
    ...
    for _name, info in SAMPLE_DATASETS.items():
...
def test_missing_value_strategy_numeric_median() -> None:
    ...
    # Act
    preprocessor, num_cols, _cat_cols = build_preprocessor(
        df, "target",
...
def test_missing_value_strategy_numeric_constant() -> None:
    ...
    # Act
    preprocessor, _num_cols, _cat_cols = build_preprocessor(
        df, "target",
...
def test_missing_value_strategy_categorical() -> None:
    ...
    # Test most_frequent
    _preprocessor, _num_cols, cat_cols = build_preprocessor(
        df, "target",
        num_impute_strategy="median",
        cat_impute_strategy="most_frequent"
    )
...
def test_model_history_tracks_last_10_models() -> None:
    ...
    assert model_history[0]["model"] == "Model_14"  # Most recent first
    assert model_history[9]["model"] == "Model_5"  # Oldest kept entry
...
def test_profile_dataset_regression() -> None:
    ...
    df = pd.DataFrame(
        {
            "feature1": [float(i) for i in range(1, 31)],
            "feature2": [float(i * 2) for i in range(1, 31)],
            "target": [float(i * 3.7) for i in range(1, 31)],  # Continuous target
        }
    )
...
def test_full_pipeline_sample_to_training() -> None:
    ...
    num_cols, _cat_cols = pd.DataFrame(), []
    from automl_app.core.helpers import quick_dtype_buckets
    num_cols, _cat_cols = quick_dtype_buckets(df, "target")
...
def test_impute_strategy_persists_across_sessions(tmp_path) -> None:
    ...
    types.ModuleType("mock_streamlit")
    mock_session_state = {}
